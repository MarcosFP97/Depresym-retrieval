...
SR_MODEL = SentenceTransformer("all-mpnet-base-v2")#, device=torch.device("mps"))
SYNT_DATA = {
  "Sadness": [
...
def search_sim_symptom(
    ...
    query_embedding = SR_MODEL.encode([query], convert_to_numpy=True, device=torch.device('mps'), normalize_embeddings=True)#.to('cuda:0')
...
args = SentenceTransformerTrainingArguments( #### pensar si tiene sentido usar WandDB
    output_dir="./models/three-epochs-sym-logging",
    num_train_epochs=3,
...
trainer.train()
SR_MODEL.save('./models/three-epochs-sym-model')
